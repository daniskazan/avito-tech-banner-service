...
func (fq *FeatureQuery) Clone() *FeatureQuery {
	...
	return &FeatureQuery{
		...
		withBanners: fq.withBanners.Clone(),
...
func (fq *FeatureQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Feature, error) {
	...
	if query := fq.withBanners; query != nil {
		if err := fq.loadBanners(ctx, query, nodes,
			func(n *Feature) { n.Edges.Banners = []*Banner{} },
			func(n *Feature, e *Banner) { n.Edges.Banners = append(n.Edges.Banners, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (fq *FeatureQuery) loadBanners(ctx context.Context, query *BannerQuery, nodes []*Feature, init func(*Feature), assign func(*Feature, *Banner)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(feature.BannersTable)
		s.Join(joinT).On(s.C(banner.FieldID), joinT.C(feature.BannersPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(feature.BannersPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(feature.BannersPrimaryKey[0]))
