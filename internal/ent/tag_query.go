...
func (tq *TagQuery) QueryBanners() *BannerQuery {
	query := (&BannerClient{config: tq.config}).Query()
...
func (tq *TagQuery) Clone() *TagQuery {
	...
	return &TagQuery{
		...
		withBanners: tq.withBanners.Clone(),
...
func (tq *TagQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tag, error) {
	...
	if query := tq.withBanners; query != nil {
		if err := tq.loadBanners(ctx, query, nodes,
			func(n *Tag) { n.Edges.Banners = []*Banner{} },
			func(n *Tag, e *Banner) { n.Edges.Banners = append(n.Edges.Banners, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TagQuery) loadBanners(ctx context.Context, query *BannerQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *Banner)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(tag.BannersTable)
		s.Join(joinT).On(s.C(banner.FieldID), joinT.C(tag.BannersPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(tag.BannersPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(tag.BannersPrimaryKey[1]))
